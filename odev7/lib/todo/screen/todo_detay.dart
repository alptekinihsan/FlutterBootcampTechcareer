              ...
              SizedBox(
                ...
                child: ElevatedButton(onPressed: (){
                  if(tfTodoAd.text.isNotEmpty && tfTodoAd.text.length<=25){
                    ...
                    context.read<TodoDetayCubit>().guncelle(widget.todo.id, tfTodoAd.text);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        backgroundColor: Colors.green,
                        content: const Text("Güncelleme Başarılı.",style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold, color: Colors.white),),
                      ),
                    );
                  }
                  else{
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        backgroundColor: Colors.green,
                        content: const Text("Karakter Uzunluğu 25'i Geçemez ve Boş Değer Girilemez !",style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold, color: Colors.white),),
                      ),
                    );
                    Navigator.pop(context,MaterialPageRoute(builder: (context)=>Anasayfa()));
                  }
